...
export default function ThreeSkillTree({ roadmap, activeWeek, onSelectWeek }) {
  ...
  const onSelectWeekRef = useRef(onSelectWeek);

  // Keep callback reference updated without triggering scene re-creation
  useEffect(() => {
    onSelectWeekRef.current = onSelectWeek;
  }, [onSelectWeek]);

  // Update node colors and emissive intensities on activeWeek changes without re-creating WebGL elements
  useEffect(() => {
    if (!nodesRef.current || nodesRef.current.length === 0) return;
    nodesRef.current.forEach((node, i) => {
      const isActive = i === activeWeek;
      const targetColor = isActive ? 0x4f46e5 : 0x1f2937;
      if (node.material) {
        node.material.color.setHex(targetColor);
        node.material.emissive.setHex(targetColor);
        node.material.emissiveIntensity = isActive ? 0.5 : 0.1;
      }
    });
  }, [activeWeek]);
  ...
  useEffect(() => {
    ...
    // Share references with external color updater
    ...
    const onMouseClick = (event) => {
      ...
      if (intersects.length > 0) {
        ...
        if (onSelectWeekRef.current) {
          onSelectWeekRef.current(node.index);
        }
      }
    };
