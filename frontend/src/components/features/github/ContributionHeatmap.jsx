...
// ── Legend ────────────────────────────────────────────────────────────────────
function Legend() {
  const shades = ['var(--color-muted)', '#014a37', '#037a5b', '#07c592', '#55ffd2'];
  return (
...
function getDarkColor(count) {
  ...
  if (count <= 3) return '#014a37';
  if (count <= 6) return '#037a5b';
  if (count <= 9) return '#07c592';
  return '#55ffd2';
}
...
export default function ContributionHeatmap({ username }) {
  ...
  if (!username) {
    return (
      <div className="w-full p-8 bg-card border border-border/80 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm">
        <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center text-primary mb-3">
          <Github size={18} />
        </div>
        <h4 className="text-sm font-bold text-foreground mb-1.5">No GitHub Username Provided</h4>
        <p className="text-xs text-muted-foreground max-w-md leading-relaxed">
          Please connect your GitHub profile in settings, or search for a username using the search input to visualize contribution DNA.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-[11px] text-[#07c592] hover:underline"
        >
