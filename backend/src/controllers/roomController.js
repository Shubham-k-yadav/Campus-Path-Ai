...
const createRoom = async (req, res) => {
  try {
    const { name, description, category, color, isPrivate, password, vibe, sessionGoal } = req.body;
...
const updateRoom = async (req, res) => {
  try {
    ...
    const room = await Room.findOneAndUpdate(
      ...
      { name, description, category, color, isPrivate, password: isPrivate ? password : null, vibe: vibe || 'default', sessionGoal: sessionGoal || '' },
      { new: true }
    );
